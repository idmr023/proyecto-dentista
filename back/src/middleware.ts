...
export async function handleRefresh(req: IncomingMessage, res: ServerResponse) {
  ...
  if (await isSessionRevoked(oldTokenHash)) {
  ...
  const db = getDb();
  const user = await db.get('SELECT id, email, role, is_active FROM users WHERE id = ?', payload.sub) as any;
  ...
  await rotateRefreshToken(oldTokenHash, user.id, newRefresh);
